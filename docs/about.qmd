...
Two molecules superposed by applying a matrix transform.
...
(
    builder.download(
        url="https://www.ebi.ac.uk/pdbe/entry-files/download/4hhb_updated.cif"
    )
    .parse(format="mmcif")
    .model_structure()
    .component()
...
(
    builder.download(
        url="https://www.ebi.ac.uk/pdbe/entry-files/download/1oj6_updated.cif"
    )
    .parse(format="mmcif")
    .model_structure()
    .transform(
...
annotation_url = (
    "https://molstar.org/mol-view-spec/examples/annotations/annotations-1h9t.cif"
)
...
Draw various geometrical primitives.
...
Load a structure and a volume from the Mol* Volume Server.
...
structure = (
    builder.download(url="https://files.rcsb.org/download/1TQN.cif.gz").parse(format="mmcif").model_structure()
)
(
...
(
    ligand.representation(type="ball_and_stick").color(
        custom={"molstar_color_theme_name": "element-symbol"}
    )
)
ligand.focus(
    up=[0.98, -0.19, 0], direction=[-28.47, -17.66, -16.32], radius=14, radius_extent=5
)
...
fo_fc.representation(type="isosurface", relative_isovalue=3, show_wireframe=True).color(
    color="green"
).opacity(opacity=0.3)
...
mvs.States(
    snapshots=[snapshot], metadata=mvs.GlobalMetadata(description="1tqn + Volume Server")
).to_dict()
